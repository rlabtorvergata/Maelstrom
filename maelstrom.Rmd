...
if (length(testfit_results) > 0) {
  ...
  cat("The best epoch was epoch n°", "with a value of MSE of", "\n\n")
}

if (length(traintest_results) > 0) {
  cat("# Forecast on observed years", "\n\n")
  cat("Test run on train and test datasets (last 5 years of the whole).", "\n\n")
  for (i in 1:length(traintest_plots)) {
    # cat("Results for", species[i])
...
if (length(f_tot) > 0) {
  ...
  if (length(pred_results) > 0) {
    ...
    for (i in 1:length(pred_results)) {
      ...
      #   cat("Comparison with Status Quo fishing mortality (in red):", "\n\n")
      #   print(plotCompare(pred_results[[i]], comp_results[[i]], i, neuralNetInputs))
      #   cat("\n\n\\newpage\n")
      # }
      # 
      #   cat("Last Predicted/Observed Year Comparison:", "\n\n")
      #   print(plotConf(subpred, neuralNetInputs, i))
      #   cat("\n\n")
